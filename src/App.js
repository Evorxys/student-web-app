...
export default function Home() {
  ...
  useEffect(() => {
    ...
    socket.current.on("receiveMessage", (data) => {
      const messageRoomId = data.message.match(/^\((\d+)\)/)?.[1];
      if (messageRoomId === roomId) {
        setTeacherMessages(`Teacher: ${data.message.replace(/^\(\d+\)Teacher: /, "")}`);
      }
    });
    return () => {
  ...
  function sendMessage() {
    if (inputText.trim() && roomId.trim()) {
      const message = `(${roomId})Student: ${inputText}`;
      setStudentMessages((prevMessages) => [...prevMessages, message]);

      if (socket.current) {
        socket.current.emit("sendMessage",`(${roomId})Student: ${inputText}`);
      }
      ...
      <Box bgColor="#2D3748" height="100vh" color="white" p={5}>
        <VStack spacing={4} align="stretch" height="100%">
          ...
          <Box
            ...
            borderRadius="md"
            p={4}
            maxHeight="300px" // Increased from 150px to 300px
            overflowY="auto"
            mb={4}
          ...
          <Box
            ...
            overflowY="auto"
            maxHeight={{ base: "200px", md: "300px" }} // Increased from 200px/300px to 400px/500px
            mb={4}
            border="2px solid #61dafb"
            borderRadius="15px"
          >
